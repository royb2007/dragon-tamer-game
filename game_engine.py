...
def ai_pick_cards(player: PlayerState, n: int, el: str) -> List[Card]:
    ...
    if strat == 'Aggressive':
        ...
        if prev_max_rank > 0:
            beaters = sorted([c for c in hand if c.effective_rank(el) > prev_max_rank],
                             key=lambda c: c.effective_rank(el))
            result = beaters[:n]
            if len(result) < n:
...
class DragonTamerGame:
    ...
    def princess_choose_tamer(self, princess_pid: str, chosen_tamer_pid: str) -> List[dict]:
        ...
        remaining_humans = [pid for pid in self._pending_love_princess_pids
        ...
        for ai_pid in list(self._pending_love_princess_pids):
            if self.players[ai_pid].is_ai:
                vote = ai_love_tamer_choice(self.players[ai_pid],
                                            self._pending_love_tamer_pids, self.players)
    ...
    def _resolve_and_finish_step(self, entries, si):
        ...
        if result.get('love_choice_needed'):
            ...
            human_princesses = [pid for pid in princess_pids if not self.players[pid].is_ai]
            ai_princesses    = [pid for pid in princess_pids if self.players[pid].is_ai]

            ai_votes = {}
            for ai_pid in ai_princesses:
                vote = ai_love_tamer_choice(self.players[ai_pid], tamer_pids, self.players)
    ...
    def _end_round(self):
        ...
        for p in self.players.values():
            if not p.out and p.dragon_count >= WIN_DRAGONS:
                ...
                return [{
                    'type': 'game_over',
                    'winner_pid': p.pid, 'winner_name': p.name,
                    'dragons': p.dragon_count, 'round': self.round,
    ...
    def assert_card_integrity(self, context='', entries=None):
        try:
            ...
            for p in self.players.values():
                label = p.name
                for c in p.hand:   register(c.cid, f'{label}.hand')
                for c in p.battle:
    ...
    def _start_arrange_phase(self):
        ...
        if not self._pending_arrange_pids:
            ...
            events.append({'type': 'phase_change', 'phase': Phase.ARRANGE_HAND,
